...
znids=$(grep "ZN" ${pdbid}_chainA_only_clean.pdb | awk '{printf "%s ", $2}')

grep "ZN"  ${pdbid}_chainA_only_clean.pdb > ZN.pdb
...
original_pdb ${pdbid}_chainA_only_clean.pdb
...
MCPB.py -i input.in -s 1

g16 <MOL_small_opt.com> MOL_small_opt.log
g16 <MOL_small_fc.com> MOL_small_fc.log
formchk MOL_small_opt.chk MOL_small_opt.fchk
g16 <MOL_large_mk.com> MOL_large_mk.log

for i in {2..4}; do MCPB.py -i input.in -s ${i}; done

tleap -s -f MOL_tleap.in
...
name $((next+2)) loop-H
"Protein-H" & !"loop-H"
name $((next+3)) Protein_H_nocatal_noloop
q
EOF
